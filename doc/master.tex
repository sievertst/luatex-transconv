...
\begin{optionlist}
  ...
  \sty{\textbackslash{}to\prm{scheme}}. In order to do this, Lua has to be able
  to find a file \prm{lang}/\prm{scheme}.lua which returns the converter. Can
...
\begin{ltxsyntax}
  ...
  the one defined by \opt{defaultscheme} or else the last scheme defined for
  \sty{lang}), using the formatting defined in \sty{\textbackslash{}langfont}.
  ...
  \sty{\textbackslash{}TransconvUseScheme} or the option \opt{scheme}. This
  command will convert \prm{text} to the \sty{scheme} using the formatting
...
though \sty{transconv} is designed with the goal of making it as painless as
possible.

As a first step, add a new file \prm{scheme}.lua in the folder \prm{lang}. The

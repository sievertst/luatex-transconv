...
The \sty{transconv} \hologo{LuaTeX} package hopes to solve these problems by
...
Also as you can see, \sty{transconv} has no problem switching back and forth
...
\sty{transconv} uses Lua code for the conversion, so it will only work with
\hologo{LuaTeX}!
...
the \sty{lua} folder) to a place where \hologo{LuaTeX} can find them. The
suggested location for the \sty{sty} file is within \sty{tex/latex/local/} in
your local \sty{texmf/} directory (probably found within your \sty{home}
directory).
...
local \sty{texmf/} directory. If for some reason, you cannot get \hologo{LuaTeX} to
...
\begin{optionlist}
  ...
  In order to successfully load a scheme, \sty{transconv} has to be able to find
...
As a first step, navigate to the directory where the \sty{transconv} lua files are

...
\begin{enumerate}
  \item They use non-ASCII characters which may be annoying to input, e.g.
    diacritics, super- or subscripts etc. Obviously can do this manually with
    \LaTeX{} commands but it can be extremely obnoxious if you have to use those
    macros very frequently.
...
This package hopes to solve these problems by
...
in the Tâi-lô scheme (\totailo{tsap8-goo7}), I would normally have to write:

\begin{lstlisting}[language={[latex]tex}]
ts\textvbaraccent{a}p-g\={o}o
\end{lstlisting}

But \sty{transconv} allows me to simply use numbers instead of the tone
...
\sty{transconv} will output the correct POJ version instead: \topoj{tsap8-goo7}.
Or maybe I'm required to use \tobp{Ban5-lam5 phing3-im1}? No problem:
\tobp{tsap8-goo7}.

Also as you can see, \sty{transconv} has no problem with you switching back and
forth between multiple different schemes, either, if that is what you need.
...
In order to be able to use \sty{transconv}, you need to copy the
\sty{transconv.sty} file as well as the \sty{transconv} directory (found inside
the \sty{lua} folder) to a place where Lua\TeX{} can find them. The suggested
location for the \sty{sty} file is within \sty{tex/latex/local/} in your local
\sty{texmf/} directory (probably found within your \sty{home} directory).

The \sty{transconv} folder can be placed in any directory in your kpathsea path.
You can check that path with the console command \sty{kpsewhich
--show-path=lua}. The suggested location is within \sty{scripts/} inside your
local \sty{texmf/} directory. If for some reason, you cannot get Lua\TeX{} to

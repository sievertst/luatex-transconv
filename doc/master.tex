...
\begin{enumerate}
  \item They use non-ASCII characters which may be annoying to input, e.g.
    ...
    manually with \hologo{LaTeX} commands but it can be extremely
    obnoxious if you
...
in the T\^{a}i-l\^{o} scheme (\totailo{tsap8-goo7}), I would normally
have to write:
...
The \sty{transconv/} folder can be placed in any directory in your
kpathsea path.
...
\hologo{TeX} formatting. Therefore, using this command means you
hard-code both the
...
\sty{scheme} package option. As with the option, each scheme should
be specified as
...
a string like
\sty{\textbackslash{}v\{{ü}\}} for instance causes problems and we
need to convert
it to \sty{\textbackslash{}v\{\textbackslash{}"\{u\}\}}. For such
cases, you can use
the variable \sty{final\textunderscore{}rep\textunderscore{}strings}
...
\subsubsection{An Example Raw Scheme: Standard Chinese
(\sty{transconv/cmn/raw.lua})}
